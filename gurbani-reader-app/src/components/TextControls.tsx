  ...
  return <div className="reader-controls">
    ...
    {panel && <div className="control-backdrop" onClick={() => setPanel(null)}><section className={`control-grid control-popover ${panel === 'layers' ? 'layers-popover' : ''}`} role="dialog" aria-modal="true" aria-label={panel === 'appearance' ? 'Appearance' : 'Reading layers'} onClick={event => event.stopPropagation()}>
      ...
      {panel === 'appearance' ? <>
        ...
        <label>Translation<select value={preferences.translationSource} onChange={event => setPreferences(current => ({ ...current, translationSource: event.target.value as ReaderPreferences['translationSource'] }))}><option value="off">Off</option><option value="banidb">BaniDB</option><option value="tggsp">TGGSP where available</option></select></label>
        <label><input type="checkbox" checked={preferences.showWordAnalysis} onChange={event => setPreferences(current => ({ ...current, showWordAnalysis: event.target.checked }))} /> TGGSP word details</label>
