...
const BHAI_GURDAS_CONTRIBUTORS = [
  "contributor:banidb:22",
  "contributor:banidb:49",
];
...
export class MobileCorpusGateway {
  ...
  async resolveCandidates(query: string): Promise<SearchCandidate[]> {
    ...
    if (/^[\u0A00-\u0A7F]+$/u.test(trimmed)) {
      return [
        {
          gurmukhi: trimmed.normalize("NFC"),
          transliteration: "",
          note: "Direct Gurmukhi query",
        },
      ];
    }
  ...
  async exactWordStats(
    ...
    const [summary, eligible, matches] = await Promise.all([
      ...
      db.query(
        `SELECT COUNT(*) AS count FROM token_occurrence t JOIN canonical_line l ON l.id = t.line_id
        WHERE t.token_class = 'lexical_gurmukhi' ${facets.clause}`,
        [...facets.params],
      ),
      db.query(
  ...
  async getLines(ang = 1, sourceWorkId = "source:G"): Promise<CanonicalLine[]> {
    ...
    const result = await db.query(
      `SELECT id, source_work_id AS sourceWorkId, text_unit_id AS textUnitId,
       ...
       FROM canonical_line WHERE source_work_id = ? AND ang = ? ORDER BY line_order LIMIT 120`,
      [sourceWorkId, ang],
    );
    ...
    const layers = await db.query(
      `SELECT id, text_unit_id AS textUnitId, content_type AS contentType, content,
       ...
       FROM provider_content WHERE text_unit_id IN (${placeholders}) ORDER BY content_type`,
      units,
    );
    ...
    for (const raw of layers.values ?? []) {
      ...
      list.push(layer);
      byUnit.set(layer.textUnitId, list);
    }
    return this.enrichTggsp(
      rows.map((row) => ({
        ...row,
        providerLayers: byUnit.get(row.textUnitId) ?? [],
      })),
    );
  }
  ...
  async getTextUnit(textUnitId: string): Promise<ShabadView> {
    ...
    const [unitResult, lineResult, layerResult] = await Promise.all([
      db.query(
        `SELECT u.id, u.source_work_id AS sourceWorkId, COALESCE(u.title, 'Shabad') AS title,
      ...
      db.query(
        `SELECT l.id, l.source_work_id AS sourceWorkId, l.text_unit_id AS textUnitId,
        ...
        WHERE l.text_unit_id = ? ORDER BY l.ang, l.line_order, l.id`,
        [textUnitId],
      ),
      db.query(
        `SELECT id, content_type AS contentType, content, attribution_label AS attributionLabel,
    ...
    const unitLines = await this.enrichTggsp(
      (lineResult.values ?? []) as unknown as CanonicalLine[],
    );
    return {
      id: String(unit.id),
      sourceWorkId: String(unit.sourceWorkId),
      title: String(unit.title),
      contributorId: String(unit.contributorId),
      contributorName: String(unit.contributorName),
      firstAng: numberValue(unit.firstAng),
      lastAng: numberValue(unit.lastAng),
      lineCount: numberValue(unit.lineCount),
      preview: unitLines[0]?.gurmukhi ?? "",
      transliteration: unitLines[0]?.transliteration ?? "",
      raag: String(unit.raag),
      lines: unitLines,
      providerLayers: (layerResult.values ?? []) as unknown as ProviderLayer[],
    };
  }

  async contributorUnits(
    ...
    const contributorClause =
      contributorId === COMBINED_BHAI_GURDAS_ID
        ? `l.contributor_id IN (${BHAI_GURDAS_CONTRIBUTORS.map(() => "?").join(",")})`
        : "l.contributor_id = ?";
    const contributorParams =
      contributorId === COMBINED_BHAI_GURDAS_ID
        ? BHAI_GURDAS_CONTRIBUTORS
        : [contributorId];
    const result = await db.query(
  ...
  async searchCorpus(
    ...
    const facets = searchFacetSql(filters, "l");
    const queryLines = (ftsQuery: string) =>
      db.query(
        `SELECT l.id AS lineId, l.text_unit_id AS textUnitId,
    ...
    let lineResult = await queryLines(
      isGurmukhi ? buildGurmukhiFtsQuery(trimmed) : buildRomanFtsQuery(trimmed),
    );
    if (!(lineResult.values ?? []).length)
      lineResult = await queryLines(
        isGurmukhi
          ? buildGurmukhiFtsQuery(trimmed, true)
          : buildRomanFtsQuery(trimmed, true),
      );
    ...
    const addScoredRow = (
      row: DatabaseRow,
      forcedScore?: number,
      forcedKind?: "first-letters",
    ) => {
      const scored = forcedKind
        ? { score: forcedScore ?? 0, kind: forcedKind }
        : scoreSearchCandidate(
            trimmed,
            String(row.gurmukhi),
      ...
      if (!current || current.score < scored.score)
        scoredRows.set(lineId, { row, score: scored.score, kind: scored.kind });
      if (!isGurmukhi)
    ...
    const initialField = isGurmukhi
      ? "idx.initials_gurmukhi"
      : "idx.initials_latin";
    const initialQuery = isGurmukhi
      ? normalizeGurmukhi(trimmed).replaceAll(" ", "")
      : normalizeRoman(trimmed).replaceAll(" ", "");
    ...
    for (const raw of initialResult.values ?? []) {
      ...
      const initials = isGurmukhi
        ? gurmukhiInitialQuery(String(row.gurmukhi))
        : latinInitialQuery(String(row.transliteration));
      addScoredRow(
        row,
        initials.startsWith(initialQuery) ? 790 : 690,
        "first-letters",
      );
    }
    ...
    const sabadResults: CorpusSearchResult[] = [...scoredRows.values()]
      .sort(
        (left, right) =>
          ...
          numberValue(left.row.ang) - numberValue(right.row.ang) ||
          numberValue(left.row.lineOrder) - numberValue(right.row.lineOrder),
      )
      .flatMap(({ row, kind, score }) => {
        ...
        return [
          {
            id: `search:sabad:${unitId}`,
            resultType: "sabad" as const,
            textUnitId: unitId,
            ...
            english: "",
            ang: numberValue(row.ang),
            contributorName: String(row.contributorName),
            lineId: String(row.lineId),
            matchKind: kind,
            providerContentTypes: splitProviderTypes(row.providerTypes),
            searchScore: score,
          },
        ];
      })
    ...
    if (isGurmukhi) {
      const forms = await db.query(
        `SELECT t.comparison_form AS gurmukhi, COUNT(*) AS frequency
        FROM token_occurrence t JOIN canonical_line l ON l.id = t.line_id
      ...
      for (const row of forms.values ?? [])
        candidateMap.set(String(row.gurmukhi), {
          gurmukhi: String(row.gurmukhi),
          transliteration: "",
          note: `${numberValue(row.frequency).toLocaleString()} exact occurrences`,
        });
    }
    ...
    const englishResult = !isGurmukhi
      ? await db.query(
          `SELECT p.id, p.text_unit_id AS textUnitId,
      ...
      GROUP BY p.id ORDER BY CASE u.source_work_id WHEN 'source:G' THEN 0 ELSE 1 END, ang LIMIT ?`,
          [
            `%${trimmed.toLowerCase()}%`,
            ...filters.providerContentTypes,
            ...englishFacets.params,
            limit,
          ],
        )
      : { values: [] };
    const translationResults: CorpusSearchResult[] = (
      englishResult.values ?? []
    ).map((row) => ({
      id: `search:translation:${String(row.id)}`,
      resultType: "translation",
      textUnitId: String(row.textUnitId),
      sourceWorkId: String(row.sourceWorkId),
      title: String(row.title),
      subtitle: `${String(row.contentType).replaceAll("_", " ")} · Ang ${numberValue(row.ang)}`,
      gurmukhi: String(row.gurmukhi),
      transliteration: "",
      english: plainText(String(row.content)),
      ang: numberValue(row.ang),
      contributorName: String(row.contributorName),
      lineId: String(row.lineId),
      matchKind: "analysis",
      providerContentTypes: splitProviderTypes(row.providerTypes).length
        ? splitProviderTypes(row.providerTypes)
        : ["tggsp-linked"],
    }));
    const linkedEnglishResult = !isGurmukhi
      ? await db.query(
          `SELECT l.id AS lineId,l.text_unit_id AS textUnitId,
      ...
      : { values: [] };
    const linkedResults: CorpusSearchResult[] = (
      linkedEnglishResult.values ?? []
    ).map((row) => ({
      id: `search:tggsp-linked:${String(row.lineId)}`,
      resultType: "translation",
      textUnitId: String(row.textUnitId),
      sourceWorkId: String(row.sourceWorkId),
      title: String(row.title),
      subtitle: `${String(row.contributorName)} · Ang ${numberValue(row.ang)} · TGGSP linked meaning`,
      gurmukhi: String(row.gurmukhi),
      transliteration: String(row.transliteration),
      english: String(row.content),
      ang: numberValue(row.ang),
      contributorName: String(row.contributorName),
      lineId: String(row.lineId),
      matchKind: "analysis",
      providerContentTypes: ["literal_translation_en"],
    }));
    const baniDbEnglishResult = !isGurmukhi
      ? await db.query(
          `SELECT l.id AS lineId,l.text_unit_id AS textUnitId,
        l.source_work_id AS sourceWorkId,l.ang,l.gurmukhi,COALESCE(l.transliteration,'') AS transliteration,
      ...
      ORDER BY CASE l.source_work_id WHEN 'source:G' THEN 0 ELSE 1 END,l.ang,l.line_order LIMIT ?`,
          [`%${trimmed.toLowerCase()}%`, ...englishFacets.params, limit],
        )
      : { values: [] };
    const baniDbResults: CorpusSearchResult[] = (
      baniDbEnglishResult.values ?? []
    ).map((row) => ({
      id: `search:banidb-translation:${String(row.lineId)}`,
      resultType: "translation",
      textUnitId: String(row.textUnitId),
      sourceWorkId: String(row.sourceWorkId),
      title: String(row.title),
      subtitle: `${String(row.contributorName)} · Ang ${numberValue(row.ang)} · BaniDB translation`,
      gurmukhi: String(row.gurmukhi),
      transliteration: String(row.transliteration),
      english: String(row.content),
      ang: numberValue(row.ang),
  ...
  async concordance(
    ...
    limit = 50,
    offset = 0,
  ): Promise<ConcordancePage> {
    const db = await this.db();
    const compare = gurmukhi.normalize("NFC");
    const facets = searchFacetSql(filters, "l");
    const [countResult, matchResult] = await Promise.all([
      db.query(
        `SELECT COUNT(DISTINCT t.line_id) AS total FROM token_occurrence t
      ...
      db.query(
        ...
        LEFT JOIN contributor c ON c.id = l.contributor_id
        WHERE t.token_class = 'lexical_gurmukhi' AND t.comparison_form = ? ${facets.clause}
        ORDER BY l.source_work_id, l.ang, l.line_order LIMIT ? OFFSET ?`,
        [compare, ...facets.params, limit, offset],
      ),
    ]);
    return {
      total: numberValue(first(countResult.values).total),
      offset,
      limit,
  ...
  async providerCoverage(): Promise<ProviderCoverage> {
    ...
    return {
      totalLayers: numberValue(row.totalLayers),
      mappedLayers: numberValue(row.mappedLayers),
      unmappedLayers: numberValue(row.unmappedLayers),
      mappedTextUnits: numberValue(row.mappedTextUnits),
      unmappedGroups: numberValue(row.unmappedGroups),
    };
  }

  async unmappedProviderAnalyses(
    limit = 20,
    offset = 0,
  ): Promise<ProviderAnalysis[]> {
    const db = await this.db();
    const groupsResult = await db.query(
      `SELECT substr(id, 1, length(id) - length(content_type) - 1) AS groupId,
        MIN(mapping_status) AS mappingStatus FROM provider_content
      WHERE text_unit_id IS NULL GROUP BY groupId ORDER BY groupId LIMIT ? OFFSET ?`,
      [limit, offset],
    );
    const groups = (groupsResult.values ?? []).map((row) => ({
      id: String(row.groupId),
      status: String(row.mappingStatus),
    }));
    ...
    return groups.map((group) => ({
      id: group.id,
      title: providerGroupTitle(group.id),
      mappingStatus: group.status,
  ...
  async relatedForms(
    ...
    const db = await this.db();
    const result = await db.query(
      `SELECT DISTINCT related.written_form AS form,
      ...
      ORDER BY related.written_form LIMIT ?`,
      [gurmukhi.normalize("NFC"), gurmukhi, limit],
    );
    const curated = (result.values ?? []).map((row) => ({
      form: String(row.form),
      relationType: String(row.relationType),
      provider: String(row.provider),
    }));
    if (curated.length >= limit) return curated;
    const base = [...gurmukhi.normalize("NFC")]
      .slice(0, Math.max(1, [...gurmukhi].length - 1))
      .join("");
    const suggestions = await db.query(
      `SELECT comparison_form AS form, COUNT(*) AS frequency
      ...
      GROUP BY comparison_form ORDER BY frequency DESC LIMIT ?`,
      [
        sourceWorkId,
        sourceWorkId,
        `${base}%`,
    ...
    return [
      ...curated,
      ...(suggestions.values ?? []).flatMap((row) => {
        const form = String(row.form);
        if (seen.has(form)) return [];
        return [
          {
            form,
            relationType: `unreviewed source-prefix candidate · ${numberValue(row.frequency).toLocaleString()} occurrences`,
            provider: "BaniDB analytical index",
          },
        ];
      }),
    ];
  ...
  async groupedFrequency(
    forms: string[],
    filters: SearchFilters = defaultSearchFilters(),
  ): Promise<GroupedFrequency> {
    const uniqueForms = [
      ...new Set(forms.map((form) => form.normalize("NFC")).filter(Boolean)),
    ];
    ...
    const [componentResult, totalResult] = await Promise.all([
      db.query(
        `SELECT t.comparison_form AS form, COUNT(*) AS frequency FROM token_occurrence t
        JOIN canonical_line l ON l.id = t.line_id
        WHERE t.token_class = 'lexical_gurmukhi' AND t.comparison_form IN (${placeholders}) ${facets.clause}
        GROUP BY t.comparison_form ORDER BY frequency DESC`,
        [...uniqueForms, ...facets.params],
      ),
      db.query(
        `SELECT COUNT(*) AS totalFrequency, COUNT(DISTINCT t.line_id) AS distinctLines,
    ...
    const counts = new Map(
      (componentResult.values ?? []).map((row) => [
        String(row.form),
    ...
    return {
      forms: uniqueForms.map((form) => ({
        form,
        frequency: counts.get(form) ?? 0,
      })),
      totalFrequency: numberValue(total.totalFrequency),
      distinctLines: numberValue(total.distinctLines),
  ...
  async raagSummaries(sourceWorkId = "source:G"): Promise<RaagSummary[]> {
    const db = await this.db();
    const result = await db.query(
      `SELECT COALESCE(raag_id, raag) AS id, raag AS name,
      ...
      GROUP BY raag_id, raag ORDER BY MIN(ang), raag`,
      [sourceWorkId, sourceWorkId],
    );
    return (result.values ?? []).map((row) => ({
      id: String(row.id),
      name: String(row.name),
      unitCount: numberValue(row.unitCount),
      lineCount: numberValue(row.lineCount),
    }));
  }

  async raagContributorSummaries(
    raag: string,
    sourceWorkId = "source:G",
  ): Promise<RaagContributorSummary[]> {
    const db = await this.db();
    const result = await db.query(
      `SELECT c.id, c.preferred_name AS name,
  ...
  async getBani(baniId: string): Promise<BaniView> {
    ...
    const lines = await this.enrichTggsp(
      (linesResult.values ?? []) as unknown as CanonicalLine[],
    );
    ...
    return {
      ...
      lines: lines.map((line, index) => ({
        ...line,
        order: numberValue(line.order),
        headerLevel: numberValue(
          (linesResult.values ?? [])[index]?.headerLevel,
        ),
        paragraphNumber:
          (linesResult.values ?? [])[index]?.paragraphNumber == null
            ? null
            : numberValue((linesResult.values ?? [])[index]?.paragraphNumber),
      })),
    };
  }
  ...
  async tggspCollections(): Promise<TggspCollectionSummary[]> {
    const db = await this.db();
    const result =
      await db.query(`SELECT c.code,c.title_en AS titleEn,c.title_pa AS titlePa,
    ...
    return (result.values ?? []).map((row) => ({
      ...
      collectionType: String(
        row.collectionType,
      ) as TggspCollectionSummary["collectionType"],
      collectionOrder: numberValue(row.collectionOrder),
  ...
  async getTggspCollection(code: string): Promise<BaniView> {
    ...
    const [collectionResult, sectionResult, lineResult] = await Promise.all([
      db.query(
        `SELECT code,title_en AS titleEn,title_pa AS titlePa,collection_type AS collectionType,
        introduction_en AS introduction,attribution_label AS attributionLabel FROM tggsp_collection WHERE code=?`,
        [code],
      ),
      db.query(
        `SELECT section_order AS 'order',title_en AS title,author_en AS author FROM tggsp_collection_section
        WHERE collection_code=? ORDER BY section_order`,
        [code],
      ),
      db.query(
        `SELECT a.id AS alignmentId,c.id,c.source_work_id AS sourceWorkId,c.text_unit_id AS textUnitId,c.ang,
        ...
        WHERE a.collection_code=? ORDER BY a.section_order,a.subsection_order,a.provider_line_order,m.member_order`,
        [code],
      ),
    ]);
    ...
    const enriched = await this.enrichTggsp(
      ordered.map((raw, index) => ({
        id: String(raw.id),
        sourceWorkId: String(raw.sourceWorkId),
        textUnitId: String(raw.textUnitId),
        order: index,
        ang: numberValue(raw.ang),
        gurmukhi: String(raw.gurmukhi),
        transliteration: String(raw.transliteration),
        contributorId: String(raw.contributorId),
        contributorName: String(raw.contributorName),
      })),
      code,
    );
    const firstOrderBySection = new Map<number, number>();
    ordered.forEach((raw, index) => {
      const order = numberValue(raw.sectionOrder);
      if (!firstOrderBySection.has(order))
        firstOrderBySection.set(order, index);
    });
    return {
      id: `tggsp:collection:${code}`,
      token: code,
      sourceWorkId: "source:G",
      gurmukhi: String(collection.titlePa),
      transliteration: preferredReadingTitle(String(collection.titleEn)),
      verseCount: enriched.length,
      attributionLabel: String(collection.attributionLabel),
      tggspAvailable: enriched.some((line) => line.tggspTranslation),
      tggspTranslatedLines: enriched.filter((line) => line.tggspTranslation)
        .length,
      introduction: String(collection.introduction),
      collectionType: String(
        collection.collectionType,
      ) as BaniView["collectionType"],
      sections: (sectionResult.values ?? []).flatMap((row) => {
        const firstLineOrder = firstOrderBySection.get(numberValue(row.order));
        return firstLineOrder === undefined
          ? []
          : [
              {
      ...
      lines: enriched.map((line, index) => ({
        ...line,
        order: index,
        headerLevel:
          firstOrderBySection.has(numberValue(ordered[index]?.sectionOrder)) &&
          firstOrderBySection.get(numberValue(ordered[index]?.sectionOrder)) ===
            index
            ? 2
            : 0,
        paragraphNumber: numberValue(ordered[index]?.sectionOrder),
      })),
    };
  }

  private async enrichTggsp(
    lines: CanonicalLine[],
    preferredCode = "",
    ...
    const canonicalIds = [
      ...new Set(
        lines
          .map((line) => line.id)
          .filter((id) => id && !id.startsWith("bani-line:")),
      ),
    ];
    if (!canonicalIds.length) return lines;
    const db = await this.db();
    const translationRows: DatabaseRow[] = [];
    const termRows: DatabaseRow[] = [];
    const bdbRows: DatabaseRow[] = [];
    ...
    for (let start = 0; start < canonicalIds.length; start += 400) {
      const batch = canonicalIds.slice(start, start + 400);
      const placeholders = batch.map(() => "?").join(",");
      const [translationResult, termResult, bdbResult] = await Promise.all([
        db.query(
          `SELECT m.canonical_line_id AS lineId,a.id AS passageId,a.anchor_line_id AS anchorId,
          ...
          WHERE visible.canonical_line_id IN (${placeholders})`,
          [...batch, ...batch],
        ),
        db.query(
          `SELECT id,collection_code AS collectionCode,canonical_line_id AS lineId,headword,transliteration,
          ...
          FROM tggsp_line_term WHERE canonical_line_id IN (${placeholders}) ORDER BY lineId,headword`,
          batch,
        ),
        db.query(
          `SELECT canonical_line_id AS lineId,content FROM line_translation WHERE provider='banidb' AND language='en' AND canonical_line_id IN (${placeholders})`,
          batch,
        ),
      ]);
      translationRows.push(...(translationResult.values ?? []));
      termRows.push(...(termResult.values ?? []));
      bdbRows.push(...(bdbResult.values ?? []));
    }
    translationRows.sort((left, right) => {
      const preferred = (row: DatabaseRow) =>
        String(row.collectionCode) === preferredCode
          ? 0
          : String(row.collectionType) === "composition"
            ? 1
            : 2;
      return (
        String(left.lineId).localeCompare(String(right.lineId)) ||
        preferred(left) - preferred(right) ||
        numberValue(left.collectionOrder) - numberValue(right.collectionOrder)
      );
    });
    const translations = new Map<string, DatabaseRow>();
    for (const row of translationRows)
      if (!translations.has(String(row.lineId)))
        translations.set(String(row.lineId), row);
    const terms = new Map<string, TggspLineTerm[]>();
    for (const row of termRows) {
      const lineId = String(row.lineId);
      const selected = translations.get(lineId);
      if (
        selected &&
        String(row.collectionCode) !== String(selected.collectionCode)
      )
        continue;
      const list = terms.get(lineId) ?? [];
      const term = {
        id: String(row.id),
        headword: String(row.headword),
        transliteration: String(row.transliteration),
        meaningEn: String(row.meaningEn),
        grammarEn: String(row.grammarEn),
        etymologyEn: String(row.etymologyEn),
        meaningPa: String(row.meaningPa),
        grammarPa: String(row.grammarPa),
        etymologyPa: String(row.etymologyPa),
      };
      if (
        !list.some(
          (item) =>
            item.headword === term.headword &&
            item.meaningEn === term.meaningEn,
        )
      )
        list.push(term);
      terms.set(lineId, list);
    }
    const bdbTranslations = new Map(
      bdbRows.map((row) => [String(row.lineId), String(row.content)]),
    );
    return lines.map((line) => {
      ...
      return {
        ...
        tggspTranslation: translation
          ? String(translation.translation)
          : undefined,
        tggspTransliteration: translation
          ? String(translation.tggspTransliteration)
          : undefined,
        tggspTranslationPa: translation
          ? String(translation.translationPa)
          : undefined,
        tggspTranslationScope: translation
          ? (String(
              translation.translationScope,
            ) as CanonicalLine["tggspTranslationScope"])
          : undefined,
        tggspCollectionCode: translation
          ? String(translation.collectionCode)
          : undefined,
        tggspPassageId: translation ? String(translation.passageId) : undefined,
        tggspPassageAnchorId: translation
          ? String(translation.anchorId)
          : undefined,
        tggspPassageMemberCount: translation
          ? numberValue(translation.memberCount)
          : undefined,
  ...
  async sources(): Promise<SourceWorkOption[]> {
    const db = await this.db();
    const result = await db.query(
      `SELECT s.id, s.title, COALESCE(MAX(l.ang), 1) AS maxAng FROM source_work s LEFT JOIN canonical_line l ON l.source_work_id = s.id GROUP BY s.id ORDER BY CASE s.id WHEN 'source:G' THEN 0 ELSE 1 END, s.title`,
    );
    return (result.values ?? []).map((row) => ({
      id: String(row.id),
      title:
        String(row.id) === "source:B"
          ? "Vaaran Bhai Gurdas Ji"
          : String(row.id) === "source:D"
            ? "Dasam Bani · selected SGPC readings"
            : String(row.title),
      maxAng: numberValue(row.maxAng),
    }));
  }
  ...
  async corpusInfo(): Promise<CorpusInfo> {
    ...
    const values = new Map(
      (metadata.values ?? []).map((row) => [
        String(row.key),
        String(row.value),
      ]),
    );
    const counts = await db.query(
      `SELECT (SELECT COUNT(*) FROM canonical_line) AS lineCount, (SELECT COUNT(*) FROM token_occurrence) AS occurrenceCount`,
    );
    const row = first(counts.values);
    return {
      buildKind: values.get("build_kind") ?? "unknown",
      corpusReleaseId: values.get("corpus_release_id") ?? "",
      providerReleaseId: values.get("provider_release_id") ?? "",
      lineCount: numberValue(row.lineCount),
      occurrenceCount: numberValue(row.occurrenceCount),
    };
  }
  ...
  async rankedForms(
    ...
    const db = await this.db();
    const result = await db.query(
      `SELECT exact_form AS form, COUNT(*) AS frequency, COUNT(DISTINCT line_id) AS distinctLines FROM token_occurrence WHERE source_work_id = ? AND token_class = 'lexical_gurmukhi' GROUP BY exact_form ORDER BY frequency DESC, exact_form LIMIT ?`,
      [sourceWorkId, limit],
    );
    return (result.values ?? []).map((row) => ({
      form: String(row.form),
      frequency: numberValue(row.frequency),
      distinctLines: numberValue(row.distinctLines),
    }));
  }
  ...
  async rankedFormsPage(
    ...
    const [count, rows] = await Promise.all([
      db.query(
        ...
        WHERE (? = 'all' OR source_work_id = ?) AND token_class = 'lexical_gurmukhi' ${clause} GROUP BY exact_form)`,
        base,
      ),
      db.query(
    ...
    return {
      total: numberValue(first(count.values).total),
      offset,
      limit,
      forms: (rows.values ?? []).map((row) => ({
        form: String(row.form),
        frequency: numberValue(row.frequency),
        distinctLines: numberValue(row.distinctLines),
      })),
  ...
  async linesByIds(ids: string[]): Promise<CanonicalLine[]> {
    ...
    const result = await db.query(
      ...
      l.line_order AS 'order', l.ang, l.gurmukhi, COALESCE(l.transliteration, '') AS transliteration,
      COALESCE(l.contributor_id, '') AS contributorId, COALESCE(c.preferred_name, '') AS contributorName
      FROM canonical_line l LEFT JOIN contributor c ON c.id = l.contributor_id WHERE l.id IN (${placeholders})`,
      ids,
    );
    const byId = new Map(
      (result.values ?? []).map((row) => [
        String(row.id),
        row as unknown as CanonicalLine,
      ]),
    );
  ...
  async textUnitsByIds(ids: string[]): Promise<TextUnitSummary[]> {
    ...
    const result = await db.query(
      ...
      WHERE u.id IN (${placeholders}) GROUP BY u.id`,
      ids,
    );
    const byId = new Map(
      (result.values ?? []).map((row) => [
  ...
  async contributorSummaries(
    limit = 1000,
    sourceWorkId = "source:G",
  ): Promise<ContributorSummary[]> {
    const db = await this.db();
    const result = await db.query(
      `SELECT c.id, c.preferred_name AS name, c.contributor_type AS type, COUNT(DISTINCT l.text_unit_id) AS unitCount, COUNT(l.id) AS lineCount FROM contributor c JOIN canonical_line l ON l.contributor_id = c.id WHERE (? = 'all' OR l.source_work_id = ?) GROUP BY c.id ORDER BY c.preferred_name COLLATE NOCASE LIMIT ?`,
      [sourceWorkId, sourceWorkId, limit],
    );
    ...
    const gurdasRows = rows.filter((row) =>
      BHAI_GURDAS_CONTRIBUTORS.includes(row.id),
    );
    if (!gurdasRows.length) return rows;
    return [
      ...rows.filter((row) => !BHAI_GURDAS_CONTRIBUTORS.includes(row.id)),
      {
        id: COMBINED_BHAI_GURDAS_ID,
        name: "Bhai Gurdas Ji",
        type: "author",
  ...
  async glossary(query: string, limit = 20): Promise<GlossaryResult[]> {
    ...
    const termResult = await db.query(
      `SELECT MIN(id) AS id,headword,MIN(transliteration) AS transliteration,
    ...
    const rows: GlossaryResult[] = (termResult.values ?? []).map((row) => ({
      ...
      etymologyPa: String(row.etymologyPa ?? ""),
      frequency: numberValue(row.frequency),
      matchKind: String(row.matchKind) as GlossaryResult["matchKind"],
      content: "",
      provider: "Sikh Research Institute — The Guru Granth Sahib Project",
      reviewStatus: "provider_published",
    }));
    if (rows.length || gurmukhi) return rows;
    const fallback = await db.query(
      `SELECT id,headword,content,provider,review_status AS reviewStatus,
    ...
    return (fallback.values ?? []).map((row) => ({
      id: String(row.id),
      headword: String(row.headword),
      content: String(row.content),
      provider: String(row.provider),
  ...
  private async tggspAvailableSearch(
    ...
    const facets = searchFacetSql(filters, "l");
    const result = await db.query(
      `SELECT u.id AS textUnitId, u.source_work_id AS sourceWorkId,
      ...
      ORDER BY MIN(l.ang), u.id LIMIT ?`,
      [...facets.params, limit],
    );
    const results: CorpusSearchResult[] = (result.values ?? []).map((row) => ({
      id: `search:tggsp-available:${String(row.textUnitId)}`,
      resultType: "translation",
      textUnitId: String(row.textUnitId),
      sourceWorkId: String(row.sourceWorkId),
      title: String(row.title),
  ...
  private async themeSearch(
    query: string,
    filters: SearchFilters,
    limit: number,
  ): Promise<CorpusSearchResponse> {
    const db = await this.db();
    const theme = resolveTggspTheme(query);
    if (!theme)
      return { query, mode: "theme", results: [], candidateForms: [] };
    const terms = theme.searchTerms;
    ...
    const providerTypeFilter = filters.providerContentTypes.length
      ...
      : "";
    const result = await db.query(
      `SELECT p.id, p.text_unit_id AS textUnitId, p.content_type AS contentType,
        ...
        COALESCE((SELECT x.transliteration FROM canonical_line x WHERE x.text_unit_id = p.text_unit_id
          ORDER BY x.ang, x.line_order LIMIT 1), '') AS transliteration
      FROM provider_content p JOIN text_unit u ON u.id = p.text_unit_id
      LEFT JOIN canonical_line l ON l.text_unit_id = u.id LEFT JOIN contributor c ON c.id = l.contributor_id
      ...
      [
        ...
        ...filters.providerContentTypes,
        ...facets.params,
        Math.max(limit * 80, 500),
      ],
    );
    const seen = new Set<string>();
    const results: CorpusSearchResult[] = (result.values ?? [])
      .map((row) => {
        const content = plainText(String(row.content));
        const matchedTerms = terms.filter((term) =>
          containsEnglishTerm(content, term),
        );
        const score =
          650 +
          matchedTerms.length * 70 +
          (containsEnglishTerm(content, query.toLowerCase().trim())
            ? 120
            : 0);
        return {
          id: `search:theme:${String(row.id)}`,
          resultType: "theme" as const,
          textUnitId: String(row.textUnitId),
          sourceWorkId: String(row.sourceWorkId),
          title: String(row.title),
          subtitle: `${theme.label} · TGGSP ${String(row.contentType).replaceAll("_", " ")} · Ang ${numberValue(row.ang)} · ${TGGSP_THEME_INDEX_VERSION}`,
          gurmukhi: String(row.gurmukhi),
          transliteration: String(row.transliteration),
          english: themeExcerpt(content, matchedTerms),
          ang: numberValue(row.ang),
          contributorName: String(row.contributorName),
          lineId: null,
          matchKind: "theme" as const,
          providerContentTypes: [String(row.contentType)],
          searchScore: score,
        };
      })
      ...
      .filter((row) => {
        if (
          (row.searchScore ?? 0) < 700 ||
          !row.textUnitId ||
          seen.has(row.textUnitId)
        )
          return false;
        seen.add(row.textUnitId);
        return true;
      })
  ...
  private async ensureSchemaV2(db: SQLiteDBConnection): Promise<void> {
    ...
    const names = new Set(
      (columns.values ?? []).map((row) => String(row.name)),
    );
    if (!names.has("raag_id"))
      await db.execute(
        `ALTER TABLE canonical_line ADD COLUMN raag_id TEXT`,
        true,
      );
    if (!names.has("raag"))
      await db.execute(`ALTER TABLE canonical_line ADD COLUMN raag TEXT`, true);
  }

  private async seedTechnicalFixture(db: SQLiteDBConnection): Promise<void> {
    const existing = await db.query(
      `SELECT COUNT(*) AS count FROM canonical_line`,
    );
    if (numberValue(first(existing.values).count) > 0) return;
    await db.run(
      `INSERT OR IGNORE INTO source_work(id, upstream_id, title, profile, corpus_release_id) VALUES (?, ?, ?, ?, ?)`,
      [
        "source:G",
        "G",
        "Guru Granth Sahib",
        "verse_corpus",
        "fixture-corpus-2026-07-12",
      ],
    );
    await db.run(
      `INSERT OR IGNORE INTO contributor(id, preferred_name, contributor_type) VALUES (?, ?, ?)`,
      ["contributor:guru-nanak-sahib", "Guru Nanak Sahib", "guru"],
    );
    await db.run(
      `INSERT OR IGNORE INTO text_unit(id, upstream_id, source_work_id, parent_id, unit_type, unit_order, title, review_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        "unit:jap",
        "fixture-jap",
        "source:G",
    ...
    for (const line of lines) {
      ...
      for (const [position, match] of tokens.entries()) {
        const exact = match[0];
        const lexical =
          /^[\u0A00-\u0A7F]+$/u.test(exact) && /[\p{L}\p{M}]/u.test(exact);
        await db.run(
          ...
          [
            ...
            exact,
            exact.normalize("NFC"),
            lexical ? "lexical_gurmukhi" : "punctuation",
            match.index,
            match.index + exact.length,
      ...
      for (const candidate of candidates) {
        await db.run(
          `INSERT OR IGNORE INTO term_form(id, glossary_entry_id, written_form, comparison_form, transliteration, relation_type)
           VALUES (?, NULL, ?, ?, ?, ?)`,
          [
            `term:${candidate.gurmukhi}`,
            candidate.gurmukhi,
            candidate.gurmukhi.normalize("NFC"),
    ...
    await db.run(
      `INSERT OR REPLACE INTO metadata(key, value) VALUES ('fixture_seeded', 'true')`,
    );
    await db.run(
      `INSERT OR REPLACE INTO metadata(key, value) VALUES ('fixture_notice', 'Technical fixture only; not a corpus release')`,
    );
  }
}
...
function textUnitSummary(row: DatabaseRow): TextUnitSummary {
  return {
    ...
    contributorName: String(row.contributorName),
    firstAng: numberValue(row.firstAng),
    lastAng: numberValue(row.lastAng),
    lineCount: numberValue(row.lineCount),
    preview: String(row.preview),
    transliteration: String(row.transliteration),
    raag: String(row.raag ?? "Unclassified"),
  };
}

function gurmukhiInitialQuery(value: string): string {
  return normalizeGurmukhi(value)
    .split(" ")
    .filter(Boolean)
    .map((word) => looseGurmukhi(word)[0] ?? "")
    .join("");
}
...
function transliterationMatches(
  transliteration: string,
  query: string,
): boolean {
  const foldedQuery = latinFold(query);
  if (foldedQuery.includes(" "))
    return (
      latinFold(transliteration).includes(foldedQuery) ||
      phoneticRoman(transliteration).includes(phoneticRoman(foldedQuery))
    );
  const wanted = foldedWord(foldedQuery);
  return latinFold(transliteration)
    .split(" ")
    .some((token) => foldedWord(token) === wanted);
}
...
function alignedCandidates(
  ...
  const gurmukhiTokens = Array.from(
    gurmukhi.matchAll(/[\p{L}\p{M}]+/gu),
    (match) => match[0],
  );
  ...
  return latinTokens.flatMap((token, index) =>
    foldedWord(token) === wanted
      ? [
          {
            gurmukhi: gurmukhiTokens[index],
            transliteration: token,
            note: "Phonetic candidate; exact forms counted separately",
          },
        ]
      : [],
  );
...
function themeExcerpt(content: string, terms: string[]): string {
  ...
  const end = Math.min(content.length, at + 260);
  return `${start ? "…" : ""}${content.slice(start, end).trim()}${end < content.length ? "…" : ""}`;
}

function containsEnglishTerm(content: string, term: string): boolean {
  const escaped = term
    .replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")
    .replace(/[-\s]+/gu, "[-\\s]+");
  return new RegExp(`(^|[^a-z])${escaped}([^a-z]|$)`, "iu").test(content);
}
