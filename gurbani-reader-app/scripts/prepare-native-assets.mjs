...
const keep = 'gurbani_reader_v10SQLite.db';
...
const removable = readdirSync(directory).filter(name => {
  ...
  return /^gurbani_reader_v(?:[5-9]|10)SQLite\.db(?:-(?:wal|shm|journal))?$/u.test(name)
    || /^\.gurbani_reader_v(?:[5-9]|10)SQLite\.db(?:\..+)?$/u.test(name)
    || /^gurbani_reader_v10SQLite\.db\.building(?:-(?:wal|shm|journal))?$/u.test(name);
});
